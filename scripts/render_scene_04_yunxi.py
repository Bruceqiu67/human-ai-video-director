...
AUDIO_FILE = os.path.join(BASE_DIR, "output", "audio", "scene_04_yunxi_master.wav")
MANIFEST_FILE = os.path.join(BASE_DIR, "output", "audio", "manifest_yunxi.json")
OUTPUT_VIDEO = os.path.join(BASE_DIR, "output", "video", "scene_04_yunxi.mp4")
QA_DIR = os.path.join(BASE_DIR, "output", "qa_frames", "scene_04_yunxi")
...
def main():
    print(f"=== 渲染第四幕 Scene 04 (云希 Yunxi · 合规道路救援版 · {TOTAL_DURATION:.2f}s · {TOTAL_FRAMES}帧) ===")
    ...
    # 动作切点
    # s4_1 & s4_2: Pose 1 耸肩错愕 (0.0s - 4.18s)
    # s4_3, s4_4, s4_5: Pose 2 托腮思考 (4.53s - 11.14s)
    # s4_6, s4_7: Pose 3 OK自信 (11.49s - 16.20s)
    f_p2_start = int(manifest[2]["start"] * FPS)
    f_p3_start = int(manifest[5]["start"] * FPS)
    f_stamp = int(manifest[1]["start"] * FPS) + 8 # 45分盖印时刻震颤
    ...
    for f in range(TOTAL_FRAMES):
        ...
        shake_x = 0
        shake_y = 0
        if f_stamp <= f <= f_stamp + 18:
            dt = f - f_stamp
            decay = math.exp(-dt * 0.22)
            shake_x = int(math.sin(dt * 1.6) * 6 * decay)
            shake_y = int(math.cos(dt * 1.3) * 5 * decay)

        scale = 1.000 + 0.035 * (f / float(TOTAL_FRAMES))
        new_w = int(WIDTH * scale)
        new_h = int(HEIGHT * scale)
        crop_x = int((new_w - WIDTH) * 0.5) + shake_x
        crop_y = int((new_h - HEIGHT) * 0.5) + shake_y
    ...
    print(f"[SUCCESS] Scene 04 云希成片完成: {OUTPUT_VIDEO}")
